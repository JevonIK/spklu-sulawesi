...
`release_manifest.json` schema 3 mengunci identitas kandidat rilis berikut:
...
Audit memuat definisi eksperimen schema 2 melalui validator yang sama dengan
command `experiment-run`. Selain jumlah skenario dan checksum, semua konektor
skenario penelitian harus dinormalisasi menjadi CCS2 dan ID skenario harus unik
lintas berkas. Laporan baru yang dihasilkan command memakai schema 3.
...
Manifest penelitian schema 1 memisahkan versi analisis 0.15.0 dari versi
